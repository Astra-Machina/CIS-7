...
int main()
{
	...
	do
	{
		...
		for (unsigned int i = 0; i < input.length(); i++)
		{
			...
			else if (c == '!' || c == 'V' || c == 'v' || c == '^')
			{
				...
				else if (c != '!')
				{
					...
					else if (!lastWasAlpha && !parenthesisEndUsed)
					{
		...
		if (!statementUsed || lastWasConnector || lastWasIf || lastWasThen || lastWasNot || parenthesisStartUsed)
		{
			cout << "The input must end with a 'statement' or a ')' else it is not a valid WFF.\n";
			isValidWFF = false;
		}
